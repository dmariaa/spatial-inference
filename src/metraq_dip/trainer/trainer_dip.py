...
class DipTrainer:
    ...
    def __do_step(self):
        ...
        self.dip_logger['train_loss'][self.K:self.K + 1, :, self.step:self.step + 1, 0] = loss['L1Loss'].detach().cpu().item()
        self.dip_logger['train_loss'][self.K:self.K + 1, :, self.step:self.step + 1, 1] = loss['MSELoss'].detach().cpu().item()

        return {'train_mae': loss['L1Loss'].item(), 'train_mse': loss['MSELoss'].item()}
