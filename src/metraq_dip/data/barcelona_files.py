...
class BarcelonaFiles:
    def __init__(
        ...
        *,
        ...
        env_dir = os.getenv("BARCELONA_DATA_DIR")
        self.data_dir = Path(env_dir) if env_dir else (data_dir or project_root / "data" / "BARCELONA")
        self.station_catalog_path = Path(
            os.getenv("BARCELONA_STATION_CATALOG") or station_catalog_path or (self.data_dir / "station_catalog.csv")
        )
        self.magnitude_catalog_path = Path(
            os.getenv("BARCELONA_MAGNITUDE_CATALOG")
            or magnitude_catalog_path
            or (self.data_dir / "magnitude_catalog.csv")
        )
    ...
    def _all_files(self) -> tuple[Path, ...]:
        files = sorted(self.data_dir.glob("barcelona_aq-*.csv"))
        if not files:
            raise FileNotFoundError(
                f"No Barcelona CSV files found in {self.data_dir}. "
                "Expected files like barcelona_aq-2024.csv"
            )
        return tuple(files)
    ...
    def _year_to_file(self) -> dict[int, Path]:
        ...
        for path in self._all_files:
            match = re.search(r"barcelona_aq-(\d{4})\.csv$", path.name)
            if match is None:
                continue
        ...
        if not mapping:
            raise FileNotFoundError(
                f"No yearly Barcelona files found in {self.data_dir}. "
                "Expected files like barcelona_aq-2024.csv"
            )
        return mapping
    ...
    def _resolve_files(
        ...
        *,
        ...
        if missing_years:
            ...
            raise FileNotFoundError(
                f"Missing Barcelona CSV files for years: {missing}. "
    ...
    def _get_sensor_catalog(self) -> pd.DataFrame:
        ...
        if not self.station_catalog_path.exists():
            raise FileNotFoundError(f"Barcelona station catalog not found: {self.station_catalog_path}")
...
barcelona_files = BarcelonaFiles()
